...
contract VaultRebalanceTest is Test {
    ...
    function _reprice(uint256 priceUsd6) internal {
    ...
    function testRebalanceNoOpWhenPriceDoesNotMove() public {
        uint256 debtBefore = vault.getCurrentDebt();
        uint256 changed = vault.rebalance();
        uint256 debtAfter = vault.getCurrentDebt();

        assertEq(changed, 0, "rebalance should be no-op without price movement");
        assertEq(debtAfter, debtBefore, "debt should not change");
    }

    function testRebalanceChangesDebtAfterLargePriceMove() public {
        uint256 debtBefore = vault.getCurrentDebt();
        ...
        uint256 debtAfter = vault.getCurrentDebt();
        assertTrue(debtAfter != debtBefore, "debt should change after price move");
    }
    ...
    function testRebalanceKeepsDebtCloseToDeltaWithinTolerance() public {
        _reprice(1_500e6);
        vault.rebalance();
        ...
        assertLe(diff * 10_000 / delta, 1_000, "debt should track delta within 10%");
    }
}
